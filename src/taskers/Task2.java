...
public class Task2 extends Thread {
    ...
    public void run() {
        ...
        doNotify("Started Task2!", runningState);
        
        for (int i = 0; i < maxValue; i++) {
            
            if (i % notifyEvery == 0) {
                doNotify("It happened in Task2: " + i, runningState);
            }
        ...
        this.runningState = RunningState.NOTRUNNING;
        doNotify("Task2 done.", runningState);
    }
    ...
    private void doNotify(String message, RunningState runningState) {
        ...
        for (Notification notification : notifications) {
            Platform.runLater(() -> {
                notification.handle(message, runningState);
            });
        }
    }
}